...
bike <- bike_raw %>%
	...
	mutate(across( # make buf equal to pl as they are both painted lanes
...
bike <- bike %>%
	mutate(
		verify_install_type = if_else(
			verify_install_type %in% c("PL", "PBL"),
			verify_install_type,
			"none"
		),
		verify_upgrade1_type = if_else(
			verify_upgrade1_type %in% c("PL", "PBL"),
			verify_upgrade1_type,
			NA
		),
		verify_upgrade2_type = if_else(
			verify_upgrade2_type %in% c("PL", "PBL"),
			verify_upgrade2_type,
			NA
		)
	)
...
# Add bike type based on year of collision
colli_bike_proc <- colli_bike %>%
	mutate( # add time units
		year = year(colli_date),
		quarter = quarter(colli_date),
	) %>%
	mutate( # add bike type
		type_year = case_when(
			year >= verify_upgrade2_year ~ verify_upgrade2_year,
			year >= verify_upgrade1_year ~ verify_upgrade1_year,
			year >= verify_install_year ~ verify_install_year,
			.default = year
		),
		type = case_when(
			year >= verify_upgrade2_year ~ verify_upgrade2_type,
			year >= verify_upgrade1_year ~ verify_upgrade1_type,
			year >= verify_install_year ~ verify_install_type,
			.default = "None"
		),
		type = case_when(
			type == "PL" ~ "painted lane",
