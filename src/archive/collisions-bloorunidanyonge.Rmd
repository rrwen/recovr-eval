...
1. Data of injury counts by infrastructure type and year
2. Plot showing injury counts by street and road user

## Data {.tabset .tabset-dropdown}

The resulting data consists of injury counts on verified bikeways by segments and by segment quarters.

**Note: Use the dropdown to view different sets of data.**
...
The quarterly rates data consists of quarterly injured individuals per segment for each street and infrastructure type:
...
* `ksi`: the total number of Killed or Seriously Injured (KSI) collisions
* `non_ksi`: the total number of non-KSI collisions
* `ksi_per_segment`: the number of Killed or Seriously Injured (KSI) collisions per segment
* `non_ksi_per_segment`: the number of non-KSI collisions per segment
* `ksi_per_segment_quarter`: the number of Killed or Seriously Injured (KSI) collisions per segment quarter
* `non_ksi_per_segment_quarter`: the number of non-KSI collisions per segment quarter
* `<USER>_ksi`: the number of KSI individuals for a road user group indicated by `<USER>`
* `<USER>_non_ksi`: the number of non-KSI individuals for a road user group indicated by `<USER>`
* `<USER>_ksi_per_segment`: the number of KSI individuals for a road user group indicated by `<USER>` per segment
* `<USER>_non_ksi_per_segment`: the number of non-KSI individuals for a road user group indicated by `<USER>` per segment
* `<USER>_ksi_per_segment_quarter`: the number of KSI individuals for a road user group indicated by `<USER>` per segment quarter
* `<USER>_non_ksi_per_segment_quarter`: the number of non-KSI individuals for a road user group indicated by `<USER>` per segment quarter

Quarterly injury rates per segment for a street and infrastructure type are calculated using the following equation:

$$
f(s,i) = \frac{\sum_{t=min\ year}^{max\ year}x_{t,s,i}/N_{s,i}}{4}
$$

* $s$ is the street
* $i$ is the infrastructure type
* $min\ year$ is the minimum year of the collision data range
* $max\ year$ is the maximum year of the collision data range
* $x_{t,s,i}$ is the number of injuries given the year $t$, street $s$, and infrastructure type $i$
* $N_{s,i}$ is the total number of bikeway segments for street $s$ and infrastructure type $i$ across $min\ year$ and $max\ year$
* $4$ is the number of quarters per year

Given the street $s$ and infrastructure type $i$, this equation sums the number of injuries $x$ between the $min\ year$ and $max\ year$, divides them the number of segments $N$, and the number 4 (the number of quarters per year).
		...
		out %>%
		    ...
		    summarize(
		        ...
		        across(
		            starts_with("ksi") | ends_with("_ksi"),
		            ~ . / segments,
		            .names = "{.col}_per_segment"
		        ),
		        across(
		        	ends_with("_per_segment"),
		        	~ . / 4,
		        	.names = "{.col}_quarter"
		        )
		    ),
