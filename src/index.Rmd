...
* `_time_month`: which month (1, 2, 3 ... 12) a bikeway had verified installation/upgrade
* `_time_quarter`: which quarter (1, 2, 3, 4) of the year a bikeway had verified installation/upgrade
* `_time_third`: which third (1, 2, 3) of the year a bikeway had verified installation/upgrade
* `_time_half`: which half (1, 2) of the year a bikeway had verified installation/upgrade
...
bike %>%
	as_tibble %>%
	select(verify_date_type, verify_date, starts_with("_")) %>%
...
# Calculate proportions with month, quarter, third, and half
bike %>%
	as_tibble %>%
	group_by(city) %>%
	summarize(
...
unit_data <- bike %>%
    ...
    mutate(
        perc_label_y = case_when(
            lag(perc) - perc < 2 ~ perc - 1.4,
            lead(perc) - perc > -2 ~ perc + 1.4,
            .default = perc
        )
    ) %>%
	group_by(city) %>%
