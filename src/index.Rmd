...
bike_sjoin <- bike_merge %>%
	...
	mutate( # add ksi aggregates
		ksi_user_driver = if_else(ksi_user %in% c("Driver", "Passenger", "Truck Driver"), 1, 0),
		ksi_user_pedestrian = if_else(ksi_user == "Pedestrian", 1, 0),
		ksi_user_cyclist = if_else(ksi_user %in% c("Cyclist", "Cyclist Passenger"), 1, 0)
	) %>%
	group_by(across(c(
		"city",
		"id",
	...
	summarize(
		across(
			-c("ksi_user_driver", "ksi_user_pedestrian", "ksi_user_cyclist"),
...
model_data <- bike_ksi %>%
	mutate( # Add whether segments were improved or not
		...
		verify_improve1_time = if_else(
			.data[[glue("_upgrade1_time_{tunit}")]] == "NA",
			NA,
			str_remove_all(.data[[glue("_upgrade1_time_{tunit}")]], "NA,|,NA")
		),
		verify_improve2 = if_else(status == "install" & verify_upgrade2_type != "NA", T, F),
		...
		verify_improve2_time = if_else(
			.data[[glue("_upgrade2_time_{tunit}")]] == "NA",
			NA,
			str_remove_all(.data[[glue("_upgrade2_time_{tunit}")]], "NA,|,NA")
		),
	...
	mutate( # assign cohort based on pl, ct, and upgrade
		...
		cohort_improve = if_else(bike_improve, cohort, NA),
		cohort_improve1 = if_else(bike_improve1, cohort, NA),
		cohort_improve2 = if_else(bike_improve2, cohort, NA)
	) %>%
	mutate( # assign post based on whether it is greater than bike time
		...
		post_improve = as.integer(time >= cohort_improve) %>% replace_na(0),
...
model_data %>% write_csv("../data/recovr-eval-2024-10-03.csv", na = "")
```
