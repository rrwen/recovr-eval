...
**Note**: Improvements can occur at different times for the same merged segment.
...
# Rowwise func for assigning improve cohort
get_cohort_improve <- function (improve_time, time_value) {
	
	# Ignore NAs
	if (is.na(improve_time)) {
		return(NA)
	}
	
	# Convert times to time ids
	improve_tid <- tibble(
		time_orig = str_split(improve_time, ",") %>% unlist
	) %>%
...
model_data <- bike_ksi %>%
	mutate( # Add whether segments were improved or not
		...
		verify_improve1_type = if_else(
			verify_upgrade1_type == "NA",
			NA,
			str_remove_all(verify_upgrade1_type, "NA,|,NA")),
		verify_improve1_year = if_else(
			verify_upgrade1_year == "NA",
			NA,
			str_remove_all(verify_upgrade1_year, "NA,|,NA")
		),
		verify_improve1_time_orig = if_else(
			.data[[glue("_upgrade1_time_{tunit}_group")]] == "NA",
			NA,
			str_remove_all(.data[[glue("_upgrade1_time_{tunit}_group")]], "NA,|,NA")
		),
		verify_improve2 = if_else(status %in% c("install", "upgrade1") & verify_upgrade2_type != "NA", T, F),
		verify_improve2_type = if_else(
			verify_upgrade2_type == "NA",
			NA,
			str_remove_all(verify_upgrade2_type, "NA,|,NA")
		),
		verify_improve2_year = if_else(
			verify_upgrade2_year == "NA",
			NA,
			str_remove_all(verify_upgrade2_year, "NA,|,NA")
		),
		verify_improve2_time_orig = if_else(
			.data[[glue("_upgrade2_time_{tunit}_group")]] == "NA",
			NA,
			str_remove_all(.data[[glue("_upgrade2_time_{tunit}_group")]], "NA,|,NA")
		),
	...
	mutate( # assign cohort based on improve
		cohort_improve1 = get_cohort_improve(bike_improve1_time_orig, time),
		cohort_improve2 = get_cohort_improve(bike_improve2_time_orig, time)
	) %>%
...
model_data %>% write_csv("../data/recovr-eval-2024-10-04.csv", na = "")
```
