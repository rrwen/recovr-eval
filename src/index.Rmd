...
# Save missing semesters for toronto
bike_nosemester %>%
	filter(city == "toronto") %>%
	select(-geometry) %>%
    write_sf("../data/archive/toronto-bikeways-nosemesterp2011-2024-06-30.csv", na = "", append = F)
bike_nosemester %>%
	filter(city == "toronto") %>%
	select(-geometry_wkb) %>%
    write_sf("../data/archive/toronto-bikeways-nosemesterp2011-2024-06-30.geojson", na = "", append = F)
