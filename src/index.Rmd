...
## Remove Same Type Upgrades {.tabset}

Set upgrades to `NA` if they are the same infrastructure type as the previous install or upgrade.

```{r}

# Remove same type upgrades
bike_removeup <- bike_combine %>%
	mutate(
		verify_upgrade2_year = if_else(
			verify_upgrade1_type == verify_upgrade2_type,
			NA,
...
cat(glue(
	...
	"\n* Columns: ", bike_combine %>% ncol,
	"\n\nAfter\n",
	"\n* Rows: ", bike_removeup %>% nrow,
	"\n* Columns: ", bike_removeup %>% ncol
))

# Display same type counts
bike_removeup_counts <- bike_combine %>%
	as_tibble %>%
	select(-geometry) %>%
	filter( # only with an install or upgrade
		!is.na(verify_install_type) &
		!is.na(verify_upgrade1_type) &
		!is.na(verify_upgrade2_type)
	) %>%
	filter( # only with same upgrades
		verify_install_type == verify_upgrade1_type |
		verify_upgrade1_type == verify_upgrade2_type
	) %>%
	mutate(
		verify_upgrades = paste0(
			verify_install_type, " -> ",
			verify_upgrade1_type, " -> ",
			verify_upgrade2_type
		)
	) %>%
	group_by(verify_upgrades) %>%
	count
bike_removeup_counts
...
**Note**: This also re-calculates the bikeway lengths after merging segments.
