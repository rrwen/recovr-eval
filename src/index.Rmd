...
model_data <- bike_ksi %>%
	mutate( # Add whether segments were improved or not
		verify_improve1 = if_else(
			status == "install" & verify_upgrade1_type != "NA",
			T,
			F
		),
		...
		verify_improve2 = if_else(
			status == "install" & verify_upgrade2_type != "NA",
			T,
			F
		),
...
model_data %>% write_csv("../data/recovr-eval-2024-10-01.csv", na = "")
```
