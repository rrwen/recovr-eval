...
`r paste0("\n * install_", colnames(vdates_raw) %>% .[. != "verify_date_raw"], collapse = "")`
`r paste0("\n * upgrade1_", colnames(vdates_raw) %>% .[. != "verify_date_raw"], collapse = "")`
`r paste0("\n * upgrade2_", colnames(vdates_raw) %>% .[. != "verify_date_raw"], collapse = "")`
...
* `_is_verify_post2011`: if a bikeway had a verified installation or upgrade after 2011
...
Then add the following time unit columns for installs/upgrades:
...
Only keep verified segments with post-2011 installs and upgrades that have dates with the chosen unit of time (`r tunit`).

```{r}

# Get time unit col
tunit_icol <- glue("install_verify_date_{tunit}")
tunit_u1col <- glue("upgrade1_verify_date_{tunit}")
tunit_u2col <- glue("upgrade2_verify_date_{tunit}")
...
bike <- bike_raw %>%
	...
	filter(
		`_is_verify_post2011` & (
			!is.na(.data[[tunit_icol]]) |
			!is.na(.data[[tunit_u1col]]) |
			!is.na(.data[[tunit_u2col]])
		 )
	)
...
bike %>%
	...
	select(
		...
		verify_upgrade1_year,
		verify_upgrade2_year,
		install_verify_date_type,
		upgrade1_verify_date_type,
		upgrade2_verify_date_type
	)
...
Pivot the data into long format, where each record represents an install or upgrade with a date range in a `r tunit` of the year.

```{r}

# Pivot install and upgrade time unit cols to long format
bike <- bike %>%
	pivot_longer(
		cols = all_of(paste0(
			c("install", "upgrade1", "upgrade2"),
			"_verify_date_",
			tunit
		)),
		names_to = "_type",
		values_to = "_time_group"
	) %>%
	filter(!is.na(`_time_group`)) # date range in time unit only

# Alter type and add time unit
bike <- bike %>%
	mutate(
		`_type` = case_when(
			str_starts(`_type`, "install") ~ "install",
			str_starts(`_type`, "upgrade1") ~ "upgrade1",
			str_starts(`_type`, "upgrade2") ~ "upgrade2"
		),
		`_time_unit` = tunit
	)
