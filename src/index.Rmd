...
vbike_nrow <- vbike_long %>%
	...
	mutate(
		`Dated Installs %` = `Dated Installs` / `Verified Dated` * 100,
		`Dated Upgrades % (1st)` = `Dated Upgrades (1st)` / `Verified Dated` * 100,
		`Dated Upgrades % (2nd)` = `Dated Upgrades (2nd)` / `Verified Dated` * 100
	) %>%
	relocate(
		"Dated Installs",
		"Dated Installs %",
...
vbike_long <- vbike_long %>%
	mutate(
		verify_date_quarter = case_when(
			month(verify_date) %in% 1:3 ~ "Q1",
			month(verify_date_start) %in% 1:3 | month(verify_date_end) %in% 1:3 ~ "Q1",
			month(verify_date) %in% 4:6 ~ "Q2",
			month(verify_date_start) %in% 4:6 | month(verify_date_end) %in% 4:6 ~ "Q2",
			month(verify_date) %in% 7:9 ~ "Q3",
			month(verify_date_start) %in% 7:9 | month(verify_date_end) %in% 7:9 ~ "Q3",
			month(verify_date) %in% 10:12 ~ "Q4",
			month(verify_date_start) %in% 10:12 | month(verify_date_end) %in% 10:12 ~ "Q4",
		)
	)
```
