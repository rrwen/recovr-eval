...
cat(glue(
	...
	"\n\nNew Columns (", bike_merge %>% ncol - bike_tfilter %>% ncol, ")\n",
	paste0(
		"\n* ",
		colnames(bike_merge) %>%
...
**Note**: Improvements and upgrades can occur at different times and parts of the same merged segment. Thus, although there is an improvement upgrade on a merged segment, it may not be present on the entire merged segment.
...
get_cohort_change <- function (
	...
	improve_tid <- tibble(
		...
		mutate(time_value = case_match(time_orig, !!!tid_map)) %>%
		pull(time_value) %>%
		as.integer
	
	# Include infra type if available
	improve_tid$type <- 
	
	# Get cohort for improve
	time_value <- time_value %>% as.integer
	if (time_value %in% improve_tid) { # time value is same as improve time
		out <- time_value
	} else if (any(improve_tid < time_value)) { # time value less than improve value
		out <- improve_tid[improve_tid < time_value] %>% max(na.rm = T)
...
model_data <- bike_ksi %>%
	mutate( # Add whether segments were improved or not
		verify_improve1 = if_else(status == "install" & verify_upgrade1_type != "NA", T, F),
		verify_improve1_type = str_remove_all(verify_upgrade1_type, "NA,|,NA"),
		verify_improve1_year = str_remove_all(verify_upgrade1_year, "NA,|,NA"),
		verify_improve1_time_orig = str_remove_all(.data[[glue("_upgrade1_time_{tunit}_group")]], "NA,|,NA"),
		verify_improve2 = if_else(status %in% c("install", "upgrade1") & verify_upgrade2_type != "NA", T, F),
		verify_improve2_type = str_remove_all(verify_upgrade2_type, "NA,|,NA"),
		verify_improve2_year = str_remove_all(verify_upgrade2_year, "NA,|,NA"),
		verify_improve2_time_orig = str_remove_all(.data[[glue("_upgrade2_time_{tunit}_group")]], "NA,|,NA"),
		verify_improve = verify_improve1 | verify_improve2
	) %>%
	mutate(across( # Set str na to actual nas
		starts_with("verify_improve"), ~ if_else(. %in% c("NA", ""), NA, .)
	)) %>%
	select(
	...
	mutate( # assign cohort based on pl, ct, and upgrade
		...
		cohort_install = if_else(bike_status == "install", cohort, NA),
		cohort_upgrade = if_else(str_starts(bike_status, "upgrade"), cohort, NA),
		cohort_upgrade1 = if_else(bike_status == "upgrade1", cohort, NA),
		cohort_upgrade2 = if_else(bike_status == "upgrade2", cohort, NA)
	) %>%
	rowwise %>%
	mutate( # assign cohort based on improve
		cohort_improve1 = get_cohort_change(bike_improve1_time_orig, time),
		cohort_improve2 = get_cohort_change(bike_improve2_time_orig, time)
	) %>%
	mutate( # assign post based on whether it is greater than bike time
		...
		post_improve = as.integer(time >= cohort_improve1 | time >= cohort_improve2) %>% replace_na(0),
