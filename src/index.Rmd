...
model_data_check_list <- list(
	...
	"Post upgrade1 is 0 after cohort time if there is no removal" = model_data %>%
		filter(
			post_upgrade1 == 0 & time >= cohort_upgrade1 & !bike_remove
		),
	"Post upgrade2 is 0 after cohort tim if there is no removal" = model_data %>%
		filter(
	...
	"Post is 1 during removal time" = model_data %>%
		filter(
			post == 1 &
			time >= cohort_remove1 &
