...
Group bikeway segments together if they are spatially adjacent and are the same infrastructure type after all installs and upgrades.

The known status is the most recent change with a known time unit for a segment with one of the following values:

* `install`: an installed segment with no upgrades
* `upgrade1`: a segment upgraded once and was previously installed
* `upgrade2`: a segment upgraded twice and was previously upgrade once and installed

This also adds an additional column `_merge_group` specifying the merge group in the format of `<CITY>_<STATUS>_<TYPE>_<ADJID>`, where:
...
bike_adj <- bike_improv %>%
    ...
    group_map(~ {
        ...
        out <- .x %>% mutate(`_adj_group` = adj_group)
        return(out)
...
cat(glue(
	...
	"\n\nNew Columns (", bike_adj %>% ncol - bike_improv %>% ncol, ")\n",
	paste0(
		"\n* ",
		colnames(bike_adj) %>%
...
```{r}
tmap_mode("view")
tm_shape(bike_adj %>% st_buffer(25)) +
    tm_polygons(
    	group = "Merged (Black)",
    	col = "black",
    	border.col = "white",
...
bike_adj_buff25 <- bike_adj %>%
	select(city, id) %>%
	st_buffer(25)

# Spatial join bikeways with ksi keeping ids
sjoin <- bike_adj_buff25 %>%
...
if (tunit != "year") {
	...
	tunit_combo <- expand_grid(
	    `_cluster_group` = unique(bike_adj$`_cluster_group`),
...
**Note 2**: If a time falls below the study period's time range, the cohort will be considered `1` (existing since the beginning of the time range).
...
get_cohort <- function (
	...
	tid_map <- Map(reformulate, shQuote(tid_df$time), shQuote(tid_df$time_orig))
	
	# Convert to char
	change_time <- change_time %>% as.character
	change_type <- change_type %>% as.character
	
	# Init change time data
	change_tid <- tibble(
	...
	# Set time data type
	class(change_tid$time_orig) <- class(tid_df$time_value)
	class(change_tid$time_value) <- class(tid_df$time_value)
	
	# Convert change times to time ids
	change_tid <- change_tid %>%
		mutate(
			time_value = if_else( # assign min if time outside range and lower
				time_orig < min(tid_df$time_value),
				min(tid_df$time),
				time_value
			)
		) %>%
	...
	if (!is.null(type_val)) {
		change_tid <- change_tid %>% filter(type %in% type_val)
	}
	...
	if (time_val %in% change_tid) { # change id same as time id
		out <- time_val
	} else if (any(change_tid < time_val)) { # if change id lt time id then use latest change id
		out <- change_tid[change_tid < time_val] %>% max(na.rm = T)
	} else { # if change id gt time id then use earliest change id
		out <- min(change_tid, na.rm = T)
	}
	return(out)
...
# Prepare model data
model_data <- bike_ksi %>%
	mutate( # add bike history in format of type(year) -> ...
	...
	mutate( # assign ids and add time units
		id = factor(bike_adj_id) %>% as.integer,
	...
	mutate( # fix cohort to min time id for times less than time range
		cohort = if_else(
			as.integer(bike_time_orig) < min(tid$time_value),
			min(tid$time),
			cohort
		)
	) %>%
	...
	mutate( # flag problematic segments
		flag_issue1_data = paste0(
			str_split(bike_install_type, ",")[[1]] %>% unique %>% paste0(., collapse=","),
			" -> ",
			str_split(bike_upgrade1_type, ",")[[1]] %>% unique %>% paste0(., collapse=","),
			" -> ",
			str_split(bike_upgrade2_type, ",")[[1]] %>% unique %>% paste0(., collapse=",")
		),
		flag_issue1 = if_else(
			flag_issue1_data %in% c(
				"PL -> CT -> PL",
				"NA -> CT -> PL",
				"CT -> PL -> NA"
			),
...
model_data %>% write_csv("../data/recovr-eval-2024-10-22.csv", na = "")
```
...
vancbike_stats <- list(
	...
	merge = bike_adj %>% filter(city == "vancouver") %>% nrow,
	...
	merge_km = calc_total_km(bike_adj %>% filter(city == "vancouver")),
...
calgbike_stats <- list(
	...
	merge = bike_adj %>% filter(city == "calgary") %>% nrow,
	...
	merge_km = calc_total_km(bike_adj %>% filter(city == "calgary")),
...
toronbike_stats <- list(
	...
	merge = bike_adj %>% filter(city == "toronto") %>% nrow,
	...
	merge_km = calc_total_km(bike_adj %>% filter(city == "toronto")),
...
digraph {
	...
	// Bikeway process
	bike_raw -> bike_verify -> bike_itypes -> bike_p2010 -> bike_tfilter -> bike_adj -> ksi_sjoin
