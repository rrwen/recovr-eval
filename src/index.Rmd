...
cat(glue(
	...
	"\n* Columns: ", bike_combine %>% ncol,
	"\n\nAfter\n",
	"\n* Rows: ", bike_ftypes %>% nrow,
	"\n* Columns: ", bike_ftypes %>% ncol
))

# Display counts
bike_ftypes_counts <- bike_ftypes %>%
	as_tibble %>%
	filter(
		verify_install_type %in% c("PL", "BUF", "PBL")
	) %>%
...
cat(glue(
	...
	paste0(
		"\n* ",
		colnames(bike_dates) %>%
			.[!. %in% colnames(bike_ftypes)],
		collapse = ""
	)
))

# Display complete date counts
bike_dates_counts <- bike_dates %>%
    ...
    mutate(
        event = case_when(
            str_starts(event, "verify_install") ~ "install",
            str_starts(event, "verify_upgrade1") ~ "upgrade1",
            str_starts(event, "verify_upgrade2") ~ "upgrade2"
        )
    ) %>%
    group_by(city, event) %>%
    summarize(
        n = sum(!is.na(event_year))
    ) %>%
	left_join(
		bike_dates %>%
		    as_tibble %>%
		    pivot_longer(
		        ...
		        names_to = "event",
		        values_to = "event_date_type"
		    ) %>%
		    mutate(
		        event = case_when(
		            str_starts(event, "_install") ~ "install",
		            str_starts(event, "_upgrade1") ~ "upgrade1",
		            str_starts(event, "_upgrade2") ~ "upgrade2"
		        )
		    ) %>%
		    group_by(city, event) %>%
		    summarize(
		        n_with_dates = sum(!is.na(event_date_type))
		    ),
		by = c("city", "event")
	) %>%
	mutate(
...
Filter for bikeways with a verified installations after 2011.
