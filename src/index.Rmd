...
sjoin_counts <- sjoin %>%
	group_by(ksi_city) %>%
	summarize(
		n_ksi_25m = n_distinct(if_else(`_sjoin_meters` <= 25, ksi_id, NA), na.rm = T),
		n_ksi_50m = n_distinct(if_else(`_sjoin_meters` <= 50, ksi_id, NA), na.rm = T),
		n_ksi_100m = n_distinct(if_else(`_sjoin_meters` <= 100, ksi_id, NA), na.rm = T),
		n_ksi_150m = n_distinct(if_else(`_sjoin_meters` <= 150, ksi_id, NA), na.rm = T),
		n_ksi_200m = n_distinct(if_else(`_sjoin_meters` <= 200, ksi_id, NA), na.rm = T),
		n_ksi_300m = n_distinct(if_else(`_sjoin_meters` <= 300, ksi_id, NA), na.rm = T),
		n_ksi_400m = n_distinct(if_else(`_sjoin_meters` <= 400, ksi_id, NA), na.rm = T),
		n_ksi_500m = n_distinct(if_else(`_sjoin_meters` <= 500, ksi_id, NA), na.rm = T)
	) %>%
	left_join(
		sjoin %>%
...
sjoin_counts %>%
	pivot_longer(cols = starts_with("n_ksi_"), names_to = "Distance") %>%
    mutate(
...
bike_sjoin_cols <- expand_grid(
	buff = c(25, 50, 100, 150, 200, 300, 400, 500),
