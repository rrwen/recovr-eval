...
```{r}
vancksi_raw <- read.table("../tmp/TAS persons involved in crashes in Vancouver 2010-2021.dat", sep = "|", header = T)
```
...
calgksi <- calgksi_raw %>%
	...
	mutate(
		ksi_city = "calgary",
		ksi_id = 1:nrow(.)  %>% as.character,
		ksi_date = as.Date(OCCURENCE_TIMESTAMP, origin = "1899-12-30"), # excel date serial numbers
...
## Filter Post-2011 Bikeways {.tabset}
...
Filter for bikeways with time unit values that were installed/upgraded post-2011.

```{r}
bike_tfilter <- bike_tunits %>%
	filter(
		(verify_install_year > 2011 & !is.na(.data[[glue("_install_time_{tunit}")]])) |
		(verify_upgrade1_year > 2011 & !is.na(.data[[glue("_upgrade1_time_{tunit}")]])) |
		(verify_upgrade2_year > 2011 & !is.na(.data[[glue("_upgrade1_time_{tunit}")]])) 
	)
```
...
model_data %>% write_csv("../data/recovr-eval-2024-09-16.csv")
```
...
vancbike_stats <- list(
	...
	p2011 = bike_p2011 %>% filter(city == "vancouver") %>% nrow,
	...
	p2011_km = calc_total_km(bike_p2011 %>% filter(city == "vancouver")),
...
calgbike_stats <- list(
	...
	p2011 = bike_p2011 %>% filter(city == "calgary") %>% nrow,
	...
	p2011_km = calc_total_km(bike_p2011 %>% filter(city == "calgary")),
...
toronbike_stats <- list(
	...
	p2011 = bike_p2011 %>% filter(city == "toronto") %>% nrow,
	...
	p2011_km = calc_total_km(bike_p2011 %>% filter(city == "toronto")),
...
digraph {
	...
	bike_p2011[
		label=<
			<b>Post-2011 Installs<br/>
			or Upgrades Only</b><br/><br/>
			Vancouver: {{vancbike_stats$p2011_km}} km (n={{vancbike_stats$p2011}}) <br/>
			Calgary: {{calgbike_stats$p2011_km}} km (n={{calgbike_stats$p2011}})<br/>
			Toronto: {{toronbike_stats$p2011_km}} km (n={{toronbike_stats$p2011}})
		>,
		pos="9,0!"
	]
	...
	// Bikeway process
	bike_raw -> bike_verify -> bike_itypes -> bike_p2011 -> bike_tfilter -> bike_merge -> ksi_sjoin
