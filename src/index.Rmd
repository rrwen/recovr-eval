...
sjoin <- bike_adj_buff25 %>%
	st_join(ksi_tunits %>% select(ksi_id)) %>%
	as_tibble %>%
	select(-geometry)
...
sjoin_counts <- sjoin %>%
	...
	left_join(
		sjoin %>%
			mutate(ksi_city = str_split_i(id, "_", 1)) %>%
			group_by(ksi_city) %>%
			summarize(
...
model_data <- bike_ksi %>%
    ...
    mutate( # Add whether segments were improved or not
        verify_improve1_type = if_else(`_improve1`, verify_upgrade1_type, NA),
        verify_improve1_year = if_else(`_improve1`, verify_upgrade1_year, NA),
        verify_improve1_time_orig = if_else(`_improve1`, .data[[glue("_upgrade1_time_{tunit}_group")]], NA),
        verify_improve2_type = if_else(`_improve2`, verify_upgrade2_type, NA),
        verify_improve2_year = if_else(`_improve2`, verify_upgrade2_year, NA),
        verify_improve2_time_orig = if_else(`_improve2`, .data[[glue("_upgrade2_time_{tunit}_group")]], NA)
    ) %>%
	select(
		bike_id = id,
		bike_adj_group = `_adj_group`,
		bike_city = city,
		bike_status = status,
		bike_type = type,
...
model_data %>% write_csv("../data/recovr-eval-2024-10-24.csv", na = "")
```
