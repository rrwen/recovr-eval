...
For the second upgrade, this is `TRUE` when the first upgrade is the same type as the second upgrade.
...
Add whether there were bikeway removals or not for each bikeway segment.
...
For the first upgrade, this is `TRUE` when the first upgrade `None` or `N`.

For the second upgrade, this is `TRUE` when the second upgrade is `None` or `N`.
...
cat(glue(
	...
	"\n\nNew Columns (", bike_adj %>% ncol - bike_remove %>% ncol, ")\n",
	paste0(
		"\n* ",
		colnames(bike_adj) %>%
			.[!. %in% colnames(bike_remove)],
		collapse = ""
	)
))
...
**Note**: Each KSI is joined to the nearest bikeway in cases where there are multiple bikeways within 25 meters.
...
sjoin <- ksi_tunits %>%
	...
	as_tibble %>%
	select(-geometry) %>%
	mutate( # remove bikeways that are not within 25 meters
		id = bike_adj_ids[sjoin_idx, ] %>% pull(id),
		id = if_else(`_sjoin_meters` <= 25, id, NA)
	) %>%
	select(id, ksi_id, ksi_city)
...
**Note**: If a time falls below the study period's time range, the cohort will be considered `1` (existing since the beginning of the time range).
...
model_data <- bike_ksi %>%
	...
	mutate( # Add whether segments were removed or not
        verify_remove1_type = if_else(`_remove1`, verify_upgrade1_type, NA),
        verify_remove1_year = if_else(`_remove1`, verify_upgrade1_year, NA),
        verify_remove1_time_orig = if_else(`_remove1`, .data[[glue("_upgrade1_time_{tunit}_group")]], NA),
        verify_remove2_type = if_else(`_remove2`, verify_upgrade2_type, NA),
        verify_remove2_year = if_else(`_remove2`, verify_upgrade2_year, NA),
        verify_remove2_time_orig = if_else(`_remove2`, .data[[glue("_upgrade2_time_{tunit}_group")]], NA)
    ) %>%
	select(
		bike_id = id,
		...
		bike_remove1_type = verify_remove1_type,
		bike_remove1_year = verify_remove1_year,
		bike_remove1_time_orig = verify_remove1_time_orig,
		bike_remove2 = `_remove2`,
		bike_remove2_type = verify_remove2_type,
		bike_remove2_year = verify_remove2_year,
		bike_remove2_time_orig = verify_remove2_time_orig,
	...
	mutate( # assign post based on whether it is greater than bike time
		...
		post_remove1 = as.integer(
			time >= cohort_remove1 & (time < cohort_upgrade2 | time < cohort_improve2)
		) %>% replace_na(0),
		post_remove1_pl = as.integer(
			time >= cohort_remove1_pl & (time < cohort_upgrade2_pl | time < cohort_improve2_pl)
		) %>% replace_na(0),
		post_remove1_ct = as.integer(
			time >= cohort_remove1_ct & (time < cohort_upgrade2_ct | time < cohort_improve2_ct)
		) %>% replace_na(0),
	...
	mutate(across( # adjust post for removals, set 0 if post_remove is 1
		...
		~ . * (+!post_remove1) * (+!post_remove2) # invert post_remove and multiply
	)) %>%
...
model_data %>% write_csv("../data/recovr-eval-2024-10-28.csv", na = "")
```
...
model_data_check_list <- list(
	...
	"Post is 1 before cohort time" = model_data %>%
		filter(
			post == 1 & time < cohort
		),
	"Post install is 0 after cohort time" = model_data %>%
		filter(
			post_install == 0 & time >= cohort_install
		),
	"Post upgrade1 is 0 after cohort time" = model_data %>%
		filter(
			post_upgrade1 == 0 & time >= cohort_upgrade1
		),
	"Post upgrade2 is 0 after cohort time" = model_data %>%
		filter(
			post_upgrade2 == 0 & time >= cohort_upgrade2
		),
...
model_data_check <- model_data_check %>%
	add_row(
		...
		pass = sum(model_data$ksi, na.rm = T) == (sjoin %>% filter(!is.na(id)) %>% pull(ksi_id) %>% unique %>% length)
	)

# Preview and save data check
model_data_check
model_data_check %>% write_csv("../data/recovr-eval-check-2024-10-28.csv", na = "")
...
vancbike_stats <- list(
	...
	sjoin = sjoin %>% filter(ksi_city == "vancouver" & !is.na(id)) %>% pull(ksi_id) %>% unique %>% length,
...
calgbike_stats <- list(
	...
	sjoin = sjoin  %>% filter(ksi_city == "calgary" & !is.na(id)) %>% pull(ksi_id) %>% unique %>% length,
...
toronbike_stats <- list(
	...
	sjoin = sjoin %>% filter(ksi_city == "toronto" & !is.na(id)) %>% pull(ksi_id) %>% unique %>% length,
