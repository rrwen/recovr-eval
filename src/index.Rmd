...
bike_nosemester %>%
	select(-geometry_wkb) %>%
    write_sf("../data/archive/bikeways-nosemester-2024-06-30.geojson", na = "", append = F)

# Save missing semesters for toronto
bike_nosemester %>%
	filter(city == "toronto") %>%
	select(-geometry) %>%
    write_sf("../data/archive/toronto-bikeways-nosemester-2024-06-30.csv", na = "", append = F)
bike_nosemester %>%
	filter(city == "toronto") %>%
	select(-geometry_wkb) %>%
    write_sf("../data/archive/toronto-bikeways-nosemester-2024-06-30.geojson", na = "", append = F)
