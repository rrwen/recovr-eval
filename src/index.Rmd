...
sjoin_counts <- sjoin %>%
	...
	group_by(city) %>%
	summarize(
		n_bike_10m = n_distinct(if_else(`_sjoin_meters` <= 10, id, NA), na.rm = T),
		n_bike_25m = n_distinct(if_else(`_sjoin_meters` <= 25, id, NA), na.rm = T),
		n_bike_50m = n_distinct(if_else(`_sjoin_meters` <= 50, id, NA), na.rm = T),
		n_bike_75m = n_distinct(if_else(`_sjoin_meters` <= 75, id, NA), na.rm = T),
		n_bike_100m = n_distinct(if_else(`_sjoin_meters` <= 100, id, NA), na.rm = T),
		n_bike_200m = n_distinct(if_else(`_sjoin_meters` <= 200, id, NA), na.rm = T),
		n_bike_300m = n_distinct(if_else(`_sjoin_meters` <= 300, id, NA), na.rm = T),
		n_bike_400m = n_distinct(if_else(`_sjoin_meters` <= 400, id, NA), na.rm = T),
		n_bike_500m = n_distinct(if_else(`_sjoin_meters` <= 500, id, NA), na.rm = T)
	) %>%
	left_join(
		sjoin %>%
...
sjoin_counts %>%
	pivot_longer(cols = starts_with("n_bike_"), names_to = "Distance") %>%
    mutate(
    	Distance = parse_number(Distance),
    	`KSI Percentage` = (value / n_ksi) * 100
    ) %>%
	rename(
		City = city,
		KSI = value
	) %>%
	ggplot(aes(x = Distance, y = KSI, color = City, label = KSI)) +
	geom_line() +
	geom_point() +
	geom_text_repel(size = 2.5) +
	labs(x = "Buffer Distance", y = "KSI Count", color = "City") +
	theme_minimal()
```
...
KSI by road users and different distances (10, 25, 50, 75, 100 .. 500 meters) to bikeways will also be counted.
...
# Set ksi buff count vars
bike_sjoin_cols <- expand_grid(
	buff = c(10, 25, 50, 75, 100, 150, 200, 300, 400, 500),
	user = c("all", "driver", "pedestrian", "cyclist", "other")
) %>%
	mutate(
		column = paste0("ksi_user_", user, "_", buff, "m")
	) %>%
	pull(buff, name = "column")

# Rejoin ksi to bike and group for counting
bike_sjoin <- bike_adj %>%
	mutate(
		id = as.character(id)
	) %>%
	...
	mutate( # calc ksi user flags without buff
	...
	add_column( # init ksi user flags within buffers
		!!!as.list(bike_sjoin_cols)
	) %>%
	mutate(across( # all ksi within buffers
		starts_with("ksi_user_all") & matches("_\\d+m$"),
		~ if_else(
			!is.na(ksi_user) & `_sjoin_meters` <= .x,
			1,
			0
		)
	)) %>%
	mutate(across( # ksi by users within buffers
		starts_with("ksi_user_") & !starts_with("ksi_user_all") & matches("_\\d+m$"),
		~ if_else(
...
model_data_check_list <- list(
	...
	"Buffered road user KSI does not add up to all KSI" = model_data %>%
		filter((if_any(
			matches("ksi_\\d+m$"),
			~.x != sum(
...
model_data_check <- model_data_check %>%
	...
	add_row(
		check = "Sum of all buffered KSIs is not equal to the total KSIs available",
		check_type = "column",
		pass = all(sapply(bike_sjoin_cols %>% unique, function(x) {
		    sum(model_data[[glue("ksi_{x}m")]], na.rm = T) == (
		        sjoin %>%
		            filter(!is.na(id), `_sjoin_meters` <= x) %>%
		            pull(ksi_id) %>%
		            unique %>%
		            length
		    )
