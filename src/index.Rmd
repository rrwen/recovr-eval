...
* `_pivot_type`: one of `LSB`, `PL`, `BUF`, `PBL`, `None` or `NA` 
...
Removes empty types from `_pivot_type` with `None` and `NA`.

```{r}

# Remove none or na bike types
bike <- bike %>%
	filter(!`_pivot_type` %in% c("None", NA))
...
## Inspect Missing Semesters {.tabset}

Display a map of post-2011 bikeways for each city with missing semesters.

* `Green`: represents installs
* `Orange``: represents 1st upgrades
* `Red`: represents 2nd upgrades

For each post-2011 bikeway, note the following columns:

* `id`: id of the cycling route
* `_pivot_event`: one of install, upgrade1 or upgrade2
* `_pivot_value`: the date of the install/upgrade, manually found during verification process
* `_time_semester`: if found and fits into a semester, the semester in form of year_part (e.g. 2019_1 means semester 1 of 2019)
* `verify_date_type`: if found, type of install/upgrade time range (semester refers to seasons of fall, winter, spring, summer on this column only)
* `verify_date_start`: if found, start date of install/upgrade extracted from `_pivot_value`
* `verify_date_end`: if found, end date of install/upgrade extracted from `_pivot_value`
...
map_nosemester <- function(city_name, df = bike_nosemester) {
	...
	# Filter for city data and remove long values
	map_data <- df %>%
	...
	tmap_mode("view")
	out <- tm_basemap("CartoDB.Positron") +
		tm_shape(
			map_data %>%
...
* [bikeways-nosemesterp2011-2024-06-30.csv](https://github.com/rrwen/recovr-eval/blob/main/data/archive/bikeways-nosemesterp2011-2024-06-30.csv)
* [bikeways-nosemesterp2011-2024-06-30.geojson](https://github.com/rrwen/recovr-eval/blob/main/data/archive/bikeways-nosemesterp2011-2024-06-30.geojson)

```{r}

# Save missing semesters csv
bike_nosemester %>%
	select(-geometry) %>%
    write_sf("../data/archive/bikeways-nosemesterp2011-2024-06-30.csv", na = "", append = F)

# Save missing semesters geojson
bike_nosemester %>%
	select(-geometry_wkb) %>%
    write_sf("../data/archive/bikeways-nosemesterp2011-2024-06-30.geojson", na = "", append = F)

# Display data
bike_nosemester %>% as_tibble
...
# Determine adjacent segments if they are within same event, type, and time unit
bike <- bike %>%
	filter(!is.na(`_time_semester_group`)) %>%
	group_by(city, `_pivot_event`, `_pivot_type`, `_time_semester_group`) %>%
	group_map(~ {
...
bike_merge <- bike %>%
	...
	summarize(
		...
		event = paste0(unique(`_pivot_event`), collapse = ","),
		type = paste0(str_to_upper(unique(`_pivot_type`)), collapse = ","),
		year = paste0(unique(`_time_year`), collapse = ","),
		semester = paste0(unique(`_time_semester`), collapse = ",")
	)
