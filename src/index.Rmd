...
sjoin <- bike_adj_buff25 %>%
	...
	as_tibble %>%
	select(-geometry) %>%
	mutate(ksi_city = str_split_i(ksi_id, "_", 1))
...
model_data <- bike_ksi %>%
	...
	mutate( # flag problematic segments
		...
		flag_issue2 = if_else(
			flag_issue2_data %in% c( # flag no installs
				"NA -> PL",
				"NA -> BUF",
				"NA -> PBL",
				"NA -> None",
		...
		flag_issue3 = if_else( # flag types not ct or pbl
			!bike_install_type %in% c("PL", "BUF", "PBL") |
			!bike_upgrade1_type %in% c("PL", "BUF", "PBL") |
			!bike_upgrade2_type %in% c("PL", "BUF", "PBL"),
			1,
			0
		)
	) %>%
...
The number of bikeways should be `0` and there should be no `ids` if all checks passed.
...
model_data_check_list <- list(
	...
	"Road user KSI does not add up to total KSI" = model_data %>%
		filter(
...
# Save data check
model_data_check %>% write_csv("../data/recovr-eval-check-2024-10-27.csv", na = "")
