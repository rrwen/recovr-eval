...
This also adds an additional column `_adj_group` specifying the adjacency group in the format of `<CITY>_<TYPE>_<ADJID>`, where:
...
model_data <- bike_ksi %>%
	...
	mutate( # fix cohort to min time id for times less than time range
		...
		cohort_install = if_else(
			!bike_improve & bike_install_year < min(tid$time_year),
			min(tid$time),
			cohort_install
		),
		cohort_upgrade1 = if_else(
			!bike_improve & bike_upgrade1_year < min(tid$time_year),
			min(tid$time),
			cohort_upgrade1
		),
		cohort_upgrade2 = if_else(
			!bike_improve & bike_upgrade2_year < min(tid$time_year),
			min(tid$time),
			cohort_upgrade2
		),
		cohort_improve1 = if_else(
