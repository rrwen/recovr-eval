...
get_cohort <- function (
	...
	# Convert time to tid
	class(time_val) <- class(tid_df$time_value)
	if (time_val < min(tid_df$time_value)) { # time id below range then set to min id
		time_val <- min(tid_df$time_value)
	} else { # time id found
		time_val <- tid_df %>% 
			filter(time_orig == time_val) %>%
			pull(time) %>%
			head(1)
	}
	
	# Get cohort for improve
	if (time_val %in% change_tid) { # time value is same as change time
		out <- time_val
	} else if (any(change_tid < time_val)) { # time value lt change value then use latest
		out <- change_tid[change_tid < time_val] %>% max(na.rm = T)
	} else { # if gt then does not exist at this time so NA
		out <- NA
	}
	return(out)
...
model_data <- bike_ksi %>%
	...
	mutate( # fix cohort to min time id for times less than time range
		cohort = if_else(
			bike_time_orig < min(tid$time_value),
			min(tid$time_value),
			cohort
		)
	) %>%
