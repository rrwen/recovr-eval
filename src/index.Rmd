	...
	df %>%
		...
		mutate( # add time groups that include the year
			...
			`_time_quarter_group` = if_else(
				!is.na(`_time_quarter`),
				glue("Quarter {`_time_quarter`} {`_year`}"),
				NA
			),
			...
			`_time_semester_group` = if_else(
				...
				glue("Semester {`_time_semester`} {`_year`}"),
				NA
			),
...
## Spatial Join Bikeways and KSI {.tabset}
...
sjoin_count <- sjoin %>%
	...
	filter(!is.na(`_ksi_time_semester`)) %>%
	group_by(city, id, `_pivot_event`, `_ksi_time_semester`) %>%
	count %>%
	ungroup %>%
	pivot_wider(
		names_from = "_ksi_time_semester",
		values_from = "n",
		names_prefix = "_ksi_time_semester_"
	)

# Add counts back into bike
bike <- bike %>% left_join(sjoin_count, by = c("city", "id", "_pivot_event"))
```

```
# Spatiotemp join for each time unit
stjid_counts <- list()
for (tunit) {
	...
	if (!file.exists(stjid_file)) {
		
		# Run spatial join if no up to date cache
		stjid <- bike %>%
			mutate(
				`_time_group` := !!sym(glue("_time_{tunit}_group"))
			) %>%
		...
		stjid %>% write_csv(stjid_file)
	
	} else {
		
		# Read spatiotemp join from cache
		stjid <- read_csv(stjid_file)
	
	}
	
	# Store ksi count per bikeway
	stjid_counts[[tunit]] <- stjid %>%
		filter(!is.na(id)) %>%
		group_by(city, id) %>%
		summarize(
			!!glue("_ksi_{tunit}_count") := n()
		) %>%
		mutate(
			city = as.character(city),
			id = as.character(id)
...
stjid_map_data <- bike %>%
	filter(city == "toronto") %>%
	st_buffer(25) %>%
	select(`_pivot_value`, starts_with("_"))
