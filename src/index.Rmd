...
model_data <- bike_ksi %>%
	...
	mutate( # flag problematic segments
		...
		flag_issue2_data = paste0(
			bike_install_type,
			" -> ",
			bike_upgrade1_type
		),
		flag_issue2 = if_else(
			flag_issue2_data %in% c(
				"NA -> PL",
				"NA -> BUF",
				"NA -> PBL",
				"NA -> None",
...
model_data %>% write_csv("../data/recovr-eval-2024-10-27.csv", na = "")
```
...
This table identifies problems stated in the `check` column.

The number of rows should be `0` and there should be no `ids` if all checks passed.


```{r}

# Check model data for logic
